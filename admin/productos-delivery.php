...
$sql = "SELECT * FROM precios_deliv_prod";
$res = mysql_query($sql);
$precios = array();
...
$sql = "SELECT * FROM categorias_deliv_prod";
$res = mysql_query($sql);
$cat = array();
while($row = mysql_fetch_assoc($res)){
	$cat[] = $row;
...
<div class="row-fluid content">
	<div class="row-fluid">
		...
		<label>Costo Delivery $ </label><input id="costo_delivery" type="text" value="<?=$config['costo_delivery']?>">
		
		<h4>Precios de Productos para Delivery</h4>
	</div>
	<div class="row-fluid">
		<table id="precios_delivery" class="table table-stripped table-hover">
			<thead>
				<th>Id</th>
...
<script>
	...
	$(document).ready(function(){
		...
		$('table#precios_delivery tr').click(function(){
			$('#id').val($(this).attr("id"));
		...
		$('#costo_delivery').change(function(){
			
			$.post('actualizar_costo_delivery.php',{"costo":$(this).val()},function(response){
				
				if (response == 'success'){
					alert("Costo del delivery actualizado con éxito");
				} else {
					alert("Hubo un problema al actualizar el Costo del delivery");
				} 
		...
		$("#actualizar").click(function(e) {
			...
			$.post('actualizar_producto.php',$("#edit-form form").serialize(),function(data){
				...
				console.log(data);
				if (data[1]=='success'){
					var datos = $('table#precios_delivery').find('tr[id="'+data[0]+'"] td');
					console.log(datos);
